...
async function testScenario(name: string, muscles: Muscle[], volume: number) {
  ...
  const computedWeights = muscles.map((m) => {
    const baseWeight = ROLE_WEIGHTS[m.role] ?? 0.5;
    const finalWeight =
      m.activationMultiplier !== 1.0 && m.activationMultiplier > 0
  ...
  console.log(`\n--- Scenario: ${name} ---`);
  console.log(
    `Total Volume: ${volume}kg | Weight Sum: ${exerciseTotalWeight.toFixed(2)}`,
  );
  for (const [mName, vol] of Object.entries(muscleVolumes)) {
    const percent = (vol / totalDistributedVolume) * 100;
    console.log(
      `${mName.padEnd(20)}: ${vol.toFixed(1)}kg (${percent.toFixed(1)}%)`,
    );
  }
}
...
async function runAllTests() {
  ...
  console.log(
    "It effectively segregates volume credit so major movers take the lead over supports.",
  );
}
