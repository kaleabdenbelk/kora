...
async function testGranularUpdateFlow() {
  ...
  const initialPlan = await planService.createCustomPlan(testUserId, {
    ...
    durationWeeks: 4,
    days: [
      { dayNumber: 1, name: "Monday Workout", exercises: [{ exerciseId: "1", sets: 3, reps: "10" }] },
      { dayNumber: 4, name: "Thursday Workout", exercises: [{ exerciseId: "2", sets: 3, reps: "10" }] },
    ],
  });

  const initialSessions = await prisma.userSession.findMany({ where: { planId: initialPlan.id } });
  console.log(`📊 Initial Sessions Count: ${initialSessions.length} (Expected 8)`);
  ...
  const updateResult = await planService.updateDayTemplate(
    ...
    1, // Monday
    {
      ...
      name: "Updated Monday",
      exercises: [
        { exerciseId: replacementEx.id, sets: 5, reps: "15", intensity: "High" }
      ]
    }
  );

  console.log(`✅ Patch completed. Updated ${updateResult.updatedSessions} individual sessions.`);

  // 4. Verification
  console.log("🧐 Verifying surgical results...");

  // Check Plan Template
  const finalPlan = await prisma.userPlan.findUnique({ where: { id: initialPlan.id } });
  const mondayTemplate = (finalPlan?.planJson as any).weeks[0].sessions.find((s: any) => s.dayNumber === 1);
  console.log(`✅ Plan Template for Day 1 updated: ${mondayTemplate.name === "Updated Monday"}`);

  // Check Sessions
  const mondaySessions = await prisma.userSession.findMany({
    where: { planId: initialPlan.id, dayNumber: 1, isDeleted: false }
  });
  const allUpdated = mondaySessions.every(s => (s.planned as any).name === "Updated Monday");
  ...
  const thursdayUntouched = thursdaySessions.every(s => (s.planned as any).name === "Thursday Workout");
  console.log(`✅ Thursday sessions remained untouched: ${thursdayUntouched}`);

  if (mondayTemplate.name === "Updated Monday" && allUpdated && thursdayUntouched) {
