  ...
  try {
    ...
    for (const ex of exercisesData) {
      ...
      if (ex.classification?.movement_pattern)
        movementPatterns.add(ex.classification.movement_pattern);
    }

    console.log(
      `💪 Seeding ${muscles.size} muscles, ${equipment.size} equipment, ${categories.size} categories and ${movementPatterns.size} patterns...`,
    );
    ...
    for (const ex of exercisesData) {
      await prisma.exercise.upsert({
        ...
        update: {
          ...
          gifUrl: ex.gifUrl,
          split: ex.classification?.split
            ? splitMap[ex.classification.split]
            : null,
          ...
          category: ex.classification?.category
            ? { connect: { name: ex.classification.category } }
            : { disconnect: true },
          movementPattern: ex.classification?.movement_pattern
            ? { connect: { name: ex.classification.movement_pattern } }
            : { disconnect: true },
        },
        create: {
          id: ex.id,
          ...
          gifUrl: ex.gifUrl,
          type:
            ex.classification?.type?.toUpperCase() === "COMPOUND"
    ...
    const rawPlans = JSON.parse(
      fs.readFileSync(path.join(LEGACY_PATH, "Ready_Kora.json"), "utf8"),
    );
