...
async function analyzeStreak(email: string) {
  ...
  console.log(
    `   Training Days:   ${user.onboarding?.trainingDaysPerWeek ?? "Not set (defaulting to 3-day grace)"}`,
  );
  ...
  for (const dateStr of sortedDates) {
    ...
    const activeMins = day.sessions.reduce(
      (a, s) => a + (s.activeMinutes ?? 0),
      0,
    );
    const totalVol = day.sessions.reduce(
    ...
    console.log(
      `   ⏱️  Active Time: ${(totalSeconds / 3600).toFixed(1)}h (${Math.round(activeMins)} active min)`,
    );
    console.log(
      `   🔥 Calories:    ${Math.round(workoutBurn)} kcal (Workout) | ${Math.round(totalBurn)} kcal (Total)`,
    );
