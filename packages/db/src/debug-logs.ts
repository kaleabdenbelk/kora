...
async function checkLatestLogs(userId: string) {
  ...
  console.log(
    `   Weights (Type: ${typeof latestLog.weightsPerSet}):`,
    latestLog.weightsPerSet,
  );
  console.log(
    `   Reps (Type: ${typeof latestLog.repsPerSet}):`,
    latestLog.repsPerSet,
  );

  const weights = latestLog.weightsPerSet;
  const _reps = latestLog.repsPerSet;
